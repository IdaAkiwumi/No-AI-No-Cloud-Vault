...
class DeCloudVault(ctk.CTk):
    def __init__(self):
        ...
        self.font_down = ctk.CTkButton(self.font_frame, text="a", width=25, height=20, command=lambda: self.change_font(-1))
        self.font_down.pack(side="left", padx=1)
        ...
        # 1. ENCRYPT SECTION
        self.enc_btn = ctk.CTkButton(self, text="🔒 ENCRYPT FILES", fg_color="#4F7942", hover_color="#3E6034", height=55, command=self.encrypt_action)
        ...
        # 2. PEEK SECTION
        self.peek_btn = ctk.CTkButton(self, text="👁️ SECURE PEEK", fg_color="#3B5335", hover_color="#2D4029", height=55, command=self.peek_action)
        ...
        # 3. DECRYPT SECTION
        self.dec_btn = ctk.CTkButton(self, text="🔓 PERMANENT RESTORE", fg_color="#333333", hover_color="#444444", height=55, command=self.decrypt_action)
        ...
        self.disclaimer_frame.pack(pady=(5, 5), padx=50, fill="x")
        ...
        self.quick_search_btn = ctk.CTkButton(self.btn_container, text="⚡ Quick Scan", fg_color="#444", width=140, height=30, command=lambda: self.search_for_key(deep=False))
        self.quick_search_btn.pack(side="left", padx=5)

        self.deep_search_btn = ctk.CTkButton(self.btn_container, text="🔍 Deep Scan", fg_color="#555", width=140, height=30, command=lambda: self.search_for_key(deep=True))
    ...
    def update_key_display(self, path):
        self.key_path = path
        display_path = (path[:15] + "..." + path[-20:]) if len(path) > 40 else path
    ...
    def change_font(self, delta):
        self.base_font_size = max(12, min(24, self.base_font_size + delta))
        self.update_font_sizes()

    def update_font_sizes(self):
        main_f = ("Arial", self.base_font_size, "bold")
        desc_f = ("Arial", max(10, self.base_font_size - 4), "italic")
        small_f = ("Arial", max(9, self.base_font_size - 6), "bold")
        warn_f = ("Arial", max(9, self.base_font_size - 5))

        self.label.configure(font=("Arial", self.base_font_size + 10, "bold"))
        ...
        self.quick_search_btn.configure(font=("Arial", self.base_font_size - 3))
        self.deep_search_btn.configure(font=("Arial", self.base_font_size - 3))
...
"4. Lost key = Lost data.", parent=self)
    ...
    def search_for_key(self, deep=False):
        ...
        lbl = ctk.CTkLabel(search_win, text=f"Scanning: {mode_text}", font=("Arial", 14, "bold"))
        lbl.pack(pady=(20, 10))
        
        p_bar = ctk.CTkProgressBar(search_win, width=300, mode="indeterminate")
        p_bar.pack(pady=10)
        p_bar.start()
        
        stats_lbl = ctk.CTkLabel(search_win, text="Initializing Scan...", font=("Arial", 12))
        stats_lbl.pack(pady=2)

        def perform_search():
            found_keys = []
            scanned_dirs = 0
            if not deep: drives = [os.path.expanduser("~")]
            else:
                if os.name == 'nt':
            ...
            for drive in drives:
                for root, dirs, files in os.walk(drive):
                    ...
                    if scanned_dirs % 20 == 0:
                        self.after(0, lambda sd=scanned_dirs: stats_lbl.configure(text=f"Folders Checked: {sd}"))
        ...
        def finalize_search(keys):
            ...
            else: messagebox.showinfo("Scan Complete", "No '.key' files with 'vault' found.", parent=self)
    ...
    def show_found_keys(self, keys):
        ...
        ctk.CTkLabel(results_win, text="Keys Located Across Drives:", font=("Arial", 14, "bold")).pack(pady=10)
        ...
        for path in keys:
            ...
            short_path = (path[:3] + "..." + path[-30:]) if len(path) > 35 else path
            lbl = ctk.CTkLabel(frame, text=short_path, font=("Arial", 10))
            lbl.pack(side="left", padx=5, pady=2)
            btn = ctk.CTkButton(frame, text="Folder", width=60, height=25, command=lambda p=path: self.open_file_folder(p))
